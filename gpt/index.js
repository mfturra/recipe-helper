...
async function fetchIngredients(user_input) {
  // try {
  const response = await openai.createCompletion({
    model:  'text-davinci-002',
...
async function fetchDirections(user_input) {
  const response = await openai.createCompletion({
    model: 'text-davinci-002',
    prompt: `Generate a professional native chef's cooking directions based on the user's protein :${user_input},
